...
(function() {
  ...
  OperationView = (function(_super) {
    ...
    OperationView.prototype.showErrorStatus = function(data, parent) {
      return parent.showStatus(data, this);
    };
    ...
    OperationView.prototype.showStatus = function(response, swaggerRequest) {
      ...
      $(".request_url", $(this.el)).html("<pre>" + url + "</pre>");
        ...
        curlHeaders = "";
        for(i = 0; i < swaggerRequest.operation.parameters.length; i++){
            param = swaggerRequest.operation.parameters[i];
            if(param.paramType == "header") {
                curlHeaders += "-H '";
                headerKey = param.name;
                curlHeaders += headerKey;
                curlHeaders += ":";
                curlHeaders += swaggerRequest.headers[headerKey];
                curlHeaders += "' ";
            }
        }

        if(swaggerRequest.operation.length > 0) {
          curlHeaders += "-H 'Content-Type:";
          curlHeaders += swaggerRequest.operation.consumes[0];
          curlHeaders += "' ";
        }


        if(swaggerRequest.params.body){
            curlHeaders += "-d '";
            curlHeaders += swaggerRequest.params.body.replace(/(\r\n|\n|\r)/gm,"");
            curlHeaders += "' ";
        }


      $(".curl", $(this.el)).html("<pre>" + "curl " + "-X " + response.method + " \"" + url + "\" " + curlHeaders + "</pre>");
